...
from app.services.lang_utils import inject_pt_lexicon, preprocess_for_vader, extract_evidence_generic, detect_lang
...
class AIService:
    async def analyze_sentiment_local(self, text: str, lang: str = "auto") -> AnalysisOutput:
        ...
        tokens = extract_evidence_generic(prep, analyzer)
        return AnalysisOutput(
            task="sentiment",
            engine="local:vader",
            result=AnalysisResult(label=label, score=float(prob), tokens=tokens),
